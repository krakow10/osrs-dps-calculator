...
- **Solver** — `solve` finds the order of attack/strength level-ups from 1/1 to 99/99 that minimizes total time (each level's exp divided by the DPS at the time it's trained), via exact DP over the level grid.
